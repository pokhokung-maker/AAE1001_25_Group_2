...
class AStarPlanner:
    ...
    def planning_with_checkpoints(self, sx, sy, gx, gy, checkpoints=None, ordered=True):
        """
        Plan a path that visits checkpoints before the final goal.

        This runs the existing A* planning() sequentially over segments:
        start -> cp1 -> cp2 -> ... -> goal and concatenates the segment paths.

        If `ordered` is False, a simple greedy nearest-neighbor ordering is used.
        """
        if not checkpoints:
        ...
        if not ordered:
            ...
            cur = (sx, sy)
            while remaining:
                nearest = min(remaining, key=lambda p: math.hypot(p[0] - cur[0], p[1] - cur[1]))
                ordered_cps.append(nearest)
                remaining.remove(nearest)
                cur = nearest
            remaining = ordered_cps

        full_rx = []
        full_ry = []
        cur_x, cur_y = sx, sy
        waypoints = remaining + [(gx, gy)]

        for wp in waypoints:
            seg_rx, seg_ry = self.planning(cur_x, cur_y, wp[0], wp[1])
            # planning returns goal->start; reverse for start->goal
            seg_x = seg_rx[::-1]
            seg_y = seg_ry[::-1]
            ...
            full_rx.extend(seg_x)
            full_ry.extend(seg_y)

            cur_x, cur_y = wp[0], wp[1]

        return full_rx, full_ry
...
def main():
    ...
    gx = 60.0  # [m]
    gy = 25.0  # [m]
    # required checkpoints (visit in this order)
    checkpoints = [(15.0, 30.0), (40.0, 20.0)]
    grid_size = 1  # [m]
    robot_radius = 1.0  # [m]
    ...
    for i in range(-10, 65): # draw the button border 
        ox.append(i)
        oy.append(-10.0)
    for i in range(-10, 65): # draw the right border
        ox.append(65.0)
        oy.append(i)
    for i in range(-10, 65): # draw the top border
        ox.append(i)
        oy.append(65.0)
    for i in range(-10, 65): # draw the left border
        ox.append(-10.0)
        oy.append(i)
    for i in range(30, 55): # draw the free border
        ox.append(10.0)
        oy.append(i)   
    ...
    if show_animation:  # pragma: no cover
        ...
        # plot checkpoints (must be visited)
        if checkpoints:
            cpx = [p[0] for p in checkpoints]
            cpy = [p[1] for p in checkpoints]
            # make checkpoints highly visible: large magenta stars with black edge
            plt.scatter(cpx, cpy, c='m', marker='*', s=160, edgecolor='k', zorder=5, label='checkpoints')
            # label checkpoints with visiting order
            for i, (cx, cy) in enumerate(checkpoints, start=1):
                plt.text(cx + 0.3, cy + 0.3, str(i), color='k', fontsize=10, weight='bold', zorder=6)
    ...
    # Plan via required checkpoints (in given order)
    rx, ry = a_star.planning_with_checkpoints(sx, sy, gx, gy, checkpoints=checkpoints, ordered=True)

    if show_animation:  # pragma: no cover
        plt.plot(rx, ry, "-r", zorder=4, label='path') # show the route 
        plt.legend()
